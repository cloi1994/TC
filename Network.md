...
<img class="inline" src="https://raw.githubusercontent.com/cloi1994/TC/master/192215h1k66aftkz4p0h6u.jpg" width="60%">

**描述一个http从发送请求到接收消息整个七层过程，用到的协议**

	A: 1.连接 当我们输入这样一个请求时，首先要建立一个socket连接，因为socket是通过ip和端口建立的，
	    所以之前还有一个DNS解析过程，把www.mycompany.com变成ip，
	   ...
	   如果找到该文件，Web服务器把该文件内容传送给相应的Web浏览器。
		...
		① HTTP 1.0 200 OK 　这是Web服务器应答的第一行，
		列出服务器正在运行的HTTP版本号和应答代码。代码"200 OK"表示请求完成。
...
**6.https 端到端中间的过程。**
	...
	A: 1、客户端告诉服务器自己支持的加密方式（对称加密）、hash算法，
	   ...
	   3、客户端验证证书
	      a) 验证证书的合法性（颁发证书的机构是否合法，证书中包含的网站地址是否与正在访问的地址一致等），
              ...
              c) 使用约定好的HASH计算握手消息，并使用生成的随机数对消息进行加密，
	         最后将之前生成的所有信息发送给网站。
...
<img class="inline" src="https://raw.githubusercontent.com/cloi1994/TC/master/4115139-f28ce41e1d7e96b2.png" width="60%">
