...
**8.为什么要有time_wait，如果没有的话 什么危害?**

	A: 可靠地实现TCP全双工连接的终止：（即在TIME_WAIT下等待2MSL，是为了尽最大努力保证四次握手正常关闭)
	   确保老的报文段在网络中消失，而不会影响新建立的连接 
	   
	   如果没有的话:
	   假设最终的 ACK 丢失 ， server 将重发 FIN ， client 必须维护 TCP 状态信息以便可以重发最终的 ACK ，
	   否则会发送RST ，结果 server 认为发生错误。 
	   TCP 实现必须可靠地终止连接的两个方向 ( 全双工关闭 ) ， client 必须进 TIME_WAIT状态，
	   因为 client 可能面临重发最终 ACK 的情形。先调用 close() 的一方会进入 TIME_WAIT 状态
