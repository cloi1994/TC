...
**8. Epoll与Select区别以及epoll优点，为什么一般情况下epoll性能比select好
	...
	A: Select, Poll: 当连接有I/O流事件产生的时候，就会去唤醒进程去处理。 
		   ...
		   但是进程并不知道是哪个连接产生的I/O流事件，于是进程就挨个不停去问：“请问是你有事要处理吗？” 浪费宝贵的CPU时间片！

	   Epoll: 当连接有I/O流事件产生的时候，epoll就会去告诉进程哪个连接有I/O流事件产生，然后进程就去处理这个进程。如此，多高效！
...
**10. Epoll ET下非阻塞读，为什么不能是阻塞**
	...
	A: 如果你的文件描述符如果不是非阻塞的.1.对于读：由于需要一直读直到把数据读完，所以大家在编写程序的时候一般会用一个循环一直读取socket，

	   那这个循环势必会在最后一次阻塞，即没有数据可读的情况下，阻塞式socket会在数据读完之后一直阻塞下去，而非阻塞式的socket则返回<0，
